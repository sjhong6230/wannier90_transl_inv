...
contains
  ...
  subroutine plot_main(atom_data, band_plot, dis_manifold, fermi_energy_list, fermi_surface_plot, &
    ...
    use w90_hamiltonian, only: hamiltonian_get_hr, hamiltonian_get_rmn, hamiltonian_setup, &
                               hamiltonian_write_hr, hamiltonian_write_tb
    ...
    integer :: nkp, bands_num_spec_points, my_node_id, i, ierr
    complex(kind=dp), allocatable :: pos_r(:, :, :, :)
    logical :: have_gamma
    ...
    ! Wigner-Seitz mapping shared by all the real-space output files
    ...
    if (output_file%write_rmn .or. output_file%write_tb) then
      allocate (pos_r(num_wann, num_wann, nrpts, 3), stat=ierr)
      if (ierr /= 0) then
        call set_error_alloc(error, 'Error in allocating pos_r in plot_main', comm)
        return
      end if

      call hamiltonian_get_rmn(kmesh_info, m_matrix, kpt_latt, real_lattice, &
                               wannier_data%centres, irvec, nrpts, output_file%transl_inv_full, &
      ...
      deallocate (pos_r, stat=ierr)
      if (ierr /= 0) then
        call set_error_dealloc(error, 'Error in deallocating pos_r in plot_main', comm)
        return
      end if
    end if
...
101 call set_error_file(error, 'Error: plot_bvec: problem opening file '//trim(seedname)//'.bvec', comm)
  ...
  !================================================!
  subroutine plot_write_rmn(pos_r, irvec, nrpts, num_wann, seedname, error, comm)
    !================================================!
    !
    ...
    ! local variables
    integer :: loop_rpt, m, n, file_unit, ierr
    character(len=33) :: header
    character(len=9)  :: cdate, ctime
    ...
    open (newunit=file_unit, file=trim(seedname)//'_r.dat', form='formatted', status='unknown', &
          iostat=ierr)
    if (ierr /= 0) then
      call set_error_file(error, 'Error: plot_write_rmn: problem opening file '//trim(seedname)//'_r.dat', comm)
      return
    end if
    ...
    do loop_rpt = 1, nrpts
      do m = 1, num_wann
        do n = 1, num_wann
          write (file_unit, '(5I5,6F12.6)') irvec(:, loop_rpt), n, m, pos_r(n, m, loop_rpt, :)
        end do
      end do
    end do
